...
def encode_and_save_batch(vae: WanVAE, clip: Optional[CLIPModel], batch: list[ItemInfo]):
    ...
    for i, item in enumerate(batch):
        if os.path.exists(item.latent_cache_path):
            print('skip, already exists:', item.latent_cache_path)
            continue
        l = latent[i]
...
import threading
def task(datasets, encode, args, cur_thread,num_threads):
    print(f"cur_thread:{cur_thread}, num_thread:{num_threads}")
    cache_latents.encode_datasets(datasets, encode, args, cur_thread=cur_thread, num_threads=num_threads)
...
def main(args):
    ...
    for i in range(num_threads):# 创建并启动线程
        t = threading.Thread(target=task, args=(datasets, encode, args, i,num_threads))
        threads.append(t)
        t.start()
