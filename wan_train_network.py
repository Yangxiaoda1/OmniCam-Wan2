...
class WanNetworkTrainer(NetworkTrainer):
    ...
    def call_dit(
        ...
        cameradict = {
            'intrinsics': [i.clone().detach().to(device=accelerator.device, dtype=network_dtype).requires_grad_(True) for i in batch['intrinsics']],
            'extrinsics': [e.clone().detach().to(device=accelerator.device, dtype=network_dtype).requires_grad_(True) for e in batch['extrinsics']],
        }
        ...
        # print(f"lat_f:{lat_f}; lat_h:{lat_h}; lat_w:{lat_w}")
        ...
        with accelerator.autocast():
            model_pred = model(noisy_model_input, t=timesteps, context=context, clip_fea=clip_fea, seq_len=seq_len, y=image_latents,cameradict=cameradict)
