...
def generate_dataset_group_by_blueprint(dataset_group_blueprint: DatasetGroupBlueprint, training: bool = False) -> DatasetGroup:
    ...
    for dataset_blueprint in dataset_group_blueprint.datasets:# 遍历 blueprint 中定义的所有数据集配置
        if dataset_blueprint.is_image_dataset:# 根据 blueprint 的标志判断当前数据集是否为图像数据集
    ...
    # assertion
    cache_directories = [dataset.cache_directory for dataset in datasets]# 对所有数据集的缓存目录进行检查，确保每个数据集的 cache_directory 是唯一的
