...
class ItemInfo:
    def __init__(
        ...
        self.camera_encoder_output_cache_path: Optional[str] = None
...
def save_latent_cache_common(item_info: ItemInfo, sd: dict[str, torch.Tensor], arch_fullname: str):
    ...
    if item_info.frame_count is not None:# 如果 item_info 包含帧数信息（例如视频数据），则将帧数添加到元数据中
    ...
    save_file(sd, item_info.latent_cache_path, metadata=metadata)# 调用 save_file 函数，将字典 sd 及元数据保存到指定的缓存路径
...
def save_camera_encoder_output_cache_wan(item_info: ItemInfo, camerainfo: dict[str, torch.Tensor]):
    """Wan architecture only. Wan2.1 only has a single text encoder"""
    item_info.camera_encoder_output_cache_path=item_info.text_encoder_output_cache_path.replace('_te','_camera')
    save_camera_encoder_output_cache_common(item_info, camerainfo, ARCHITECTURE_WAN_FULL)



def save_camera_encoder_output_cache_common(item_info: ItemInfo, sd: dict[str, torch.Tensor], arch_fullname: str):
    for key, value in sd.items():
        # NaN check and show warning, replace NaN with 0
        if torch.isnan(value).any():
            logger.warning(f"{key} tensor has NaN: {item_info.item_key}, replace NaN with 0")
    ...
    if os.path.exists(item_info.camera_encoder_output_cache_path):
        # load existing cache and update metadata
        with safetensors_utils.MemoryEfficientSafeOpen(item_info.camera_encoder_output_cache_path) as f:
            existing_metadata = f.metadata()
            for key in f.keys():
                if key not in sd:  # avoid overwriting by existing cache, we keep the new one
                    sd[key] = f.get_tensor(key)
        ...
        camera_encoder_output_dir = os.path.dirname(item_info.camera_encoder_output_cache_path)
        os.makedirs(camera_encoder_output_dir, exist_ok=True)
    
    
    safetensors_utils.mem_eff_save_file(sd, item_info.camera_encoder_output_cache_path, metadata=metadata)
...
class BucketBatchManager:
    ...
    def __getitem__(self, idx):
        ...
        for item_info in bucket[start:end]:
            ...
            sd_camera=load_file(item_info.camera_encoder_output_cache_path)
            # print('sd_camera: ',sd_camera)
            # print('type_sd_te:',type(sd_te['varlen_t5_bfloat16']))
            # print('type_sd_camera:',type(sd_camera['varlen_camera_float32']))
            sd = {**sd_latent, **sd_te, **sd_camera}# 合并以上三个数据字典到一个字典中
            # print('晓达到此一游')
            # print('sd_te.keys',sd_te.keys())
            # print('sd_latent.keys',sd_latent.keys())
            # print('sd_camera.keys',sd_camera.keys())

            # TODO refactor this
            for key in sd.keys():# 遍历合并后的字典 sd 中的每个 key
                is_varlen_key = key.startswith("varlen_")  # 判断当前 key 是否为变长数据，变长数据的 key 通常以 "varlen_" 开头
                if "extrinsics" in key or "intrinsics" in key:
                    is_varlen_key=True
                content_key = key# 初始化内容 key 为原始 key

                if is_varlen_key:# 如果是变长 key，则去掉 "varlen_" 前缀
                    content_key = content_key.replace("varlen_", "")

                if content_key.endswith("_mask"):# 如果 key 是 mask 数据，则直接跳过
                    pass
                else:
                    content_key = content_key.rsplit("_", 1)[0]   # 对非 mask 数据，去掉末尾的 dtype 后缀
                    if content_key.startswith("latents_"): # 如果 key 以 "latents_" 开头，则进一步去掉代表尺寸（FxHxW）的部分
                        content_key = content_key.rsplit("_", 1)[0]  # remove FxHxW

                if content_key not in batch_tensor_data:# 如果字典中尚未存在该内容 key，则初始化为一个空列表
                    batch_tensor_data[content_key] = []
                batch_tensor_data[content_key].append(sd[key])# 将当前 key 对应的数据添加到列表中
                if is_varlen_key:
                    varlen_keys.add(content_key)# 如果当前 key 是变长的，则记录该 content_key
        # from IPython import embed;embed()
        for key in batch_tensor_data.keys():
            if key not in varlen_keys:
                # print("key:",key)
                # print("len(batch_tensor_data[key]):",len(batch_tensor_data[key]))
                batch_tensor_data[key] = torch.stack(batch_tensor_data[key])

        # print('batch_tensor_data.keys:',batch_tensor_data.keys())
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def get_all_camera_encoder_output_cache_files(self):
        return glob.glob(os.path.join(self.cache_directory, f"*_{self.architecture}_camera.safetensors"))
    ...
    def get_text_encoder_output_cache_path(self, item_info: ItemInfo) -> str:
        ...
        basename = os.path.splitext(os.path.basename(item_info.item_key))[0] # 从 item_info.item_key 中提取文件名
        assert self.cache_directory is not None, "cache_directory is required / cache_directoryは必須です"
        return os.path.join(self.cache_directory, f"{basename}_{self.architecture}_te.safetensors")# 拼接 cache_directory 与生成的文件名，文件名格式为：basename_architecture_te.safetensors
...
class VideoDataset(BaseDataset):
    ...
    def prepare_for_training(self):
        ...
        for cache_file in latent_cache_files:
            ...
            text_encoder_output_cache_file = os.path.join(self.cache_directory, f"{item_key}_{self.architecture}_te.safetensors")
            camera_encoder_output_cache_file = os.path.join(self.cache_directory, f"{item_key}_{self.architecture}_camera.safetensors")
            if not os.path.exists(text_encoder_output_cache_file):
                logger.warning(f"Text encoder output cache file not found: {text_encoder_output_cache_file}")
                continue
            if not os.path.exists(camera_encoder_output_cache_file):
                logger.warning(f"Text encoder output cache file not found: {camera_encoder_output_cache_file}")
                continue
            ...
            # from IPython import embed;embed()
            item_info.text_encoder_output_cache_path = text_encoder_output_cache_file
            item_info.camera_encoder_output_cache_path = camera_encoder_output_cache_file
