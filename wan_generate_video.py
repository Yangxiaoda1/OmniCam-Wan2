...
import logging
from myintrinsics_dust3r import get_intrinsics_from_image
...
def merge_lora_weights(model: WanModel, args: argparse.Namespace, device: torch.device) -> None:
    ...
    for i, lora_weight in enumerate(args.lora_weight):
        ...
        current_lora_params = sum(p.numel() for p in weights_sd.values())
        total_lora_params += current_lora_params
    ...
    # print(f"总的 LoRA 可训练参数数量：{total_lora_params}")
...
def prepare_i2v_inputs(
    ...
    if "#trajectory" in prompts:
        ...
        cameradict=load_file(camerapath)
        for key,value in cameradict.items():
            cameradict[key]=value.to(torch.bfloat16).unsqueeze(0).to(device) #.to(torch.bfloat16).detach().cpu()
        # from IPython import embed;embed()
    else:
        intrinsics = get_intrinsics_from_image(imagepath)
        # from IPython import embed;embed()
        embed=process_ss(prompts)
        embed=torch.tensor(embed).to('cuda')
        cameradict={
            'intrinsics': torch.tensor(intrinsics).unsqueeze(0),
            'extrinsics': embed.unsqueeze(0),
        }
        # from IPython import embed;embed()
        cameradict = {
            'intrinsics': [torch.tensor(i).to(device=accelerator.device) for i in cameradict['intrinsics']],
            'extrinsics': [torch.tensor(e).to(device=accelerator.device) for e in cameradict['extrinsics']],
        }
