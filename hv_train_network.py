...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        def set_req_grad_for_specific_params(module, param_names, flag=True):
            for name, param in module.named_parameters():
                if any(param_name in name for param_name in param_names):
                    param.requires_grad = flag
        def set_req_grad(module, flag=True):
            for param in module.parameters():
                param.requires_grad = flag
        param_names = [
            'cross_attn.conv', 'cross_attn.k_camera1', 'cross_attn.v_camera1',
            'cross_attn.norm_k_cam1'
        ]
        for blk in transformer.blocks:
            set_req_grad_for_specific_params(blk, param_names, True)
        set_req_grad(transformer.myadapter, True)
        # from IPython import embed;embed()
        ...
        for epoch in range(epoch_to_start, num_train_epochs):
                ...
                if accelerator.sync_gradients:
                    ...
                    if should_sampling or should_saving:
                        ...
                        if should_saving:
                            ...
                            if accelerator.is_main_process:
                                save_path = f"{args.output_dir}/{global_step+1}.safetensors"
                                orig_model = transformer.module
            ...
            if args.save_every_n_epochs is not None:
                ...
                if is_main_process and saving:
                    save_path = f"{args.output_dir}/total-{epoch+1}.safetensors"
                    os.makedirs(os.path.dirname(save_path), exist_ok=True)
                    # save_path = f"/data/musubi-tuner/weights/mycheckpoint/totckpt-OmniCam-Sep/{epoch+1}.safetensors"
                    orig_model = transformer.module
                    state_dict = orig_model.state_dict()
                    # from IPython import embed;embed()
                    save_file(state_dict, save_path)
        ...
        if is_main_process:
            save_path = f"{args.output_dir}/total.safetensors"
            # from IPython import embed;embed()
            os.makedirs(args.output_dir, exist_ok=True)
            orig_model = transformer.module
            state_dict = orig_model.state_dict()
