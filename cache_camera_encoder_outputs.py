...
def prepare_cache_files_and_paths_camera(datasets: list[BaseDataset]):
    all_cache_files_for_dataset = []  # exisiting cache files
    all_cache_paths_for_dataset = []  # all cache paths in the dataset
    for dataset in datasets:
        all_cache_files = [os.path.normpath(file) for file in dataset.get_all_camera_encoder_output_cache_files()]
        all_cache_files = set(all_cache_files)
        all_cache_files_for_dataset.append(all_cache_files)
...
def process_camera_encoder_batches(
    num_workers: Optional[int],
    skip_existing: bool,
    batch_size: int,
    datasets: list[BaseDataset],
    ...
    for i, dataset in enumerate(datasets):
        ...
        for batch in tqdm(dataset.retrieve_text_encoder_output_cache_batches(num_workers)):
            # update cache files (it's ok if we update it multiple times)
            # from IPython import embed;embed()
            all_cache_paths.update([os.path.normpath(item.text_encoder_output_cache_path.replace('_te','_camera')) for item in batch])
            # from IPython import embed;embed()
            # skip existing cache files
            if skip_existing:
                filtered_batch = [
                    item for item in batch if not os.path.normpath(item.text_encoder_output_cache_path.replace('_te','_camera')) in all_cache_files
                ]
                # print(f"Filtered {len(batch) - len(filtered_batch)} existing cache files")
                if len(filtered_batch) == 0:
                    continue
...
def main(args):
    ...
    # Load dataset config
    blueprint_generator = BlueprintGenerator(ConfigSanitizer())# 创建基础参数解析器
    logger.info(f"Load dataset config from {args.dataset_config}")
    user_config = config_utils.load_user_config(args.dataset_config)# 从配置文件中加载用户配置
    blueprint = blueprint_generator.generate(user_config, args, architecture=ARCHITECTURE_HUNYUAN_VIDEO)# 生成 blueprint 配置，传入用户配置和命令行参数，同时指定架构类型
    train_dataset_group = config_utils.generate_dataset_group_by_blueprint(blueprint.dataset_group)# 根据 blueprint 生成数据集分组

    datasets = train_dataset_group.datasets# 获取数据集列表

    # define accelerator for fp8 inference
    accelerator = None
    if args.fp8_llm:# 如果使用 fp8 模式，则创建 accelerate 实例，用于混合精度推理
