...
def encode_datasets(datasets: list[BaseDataset], encode: callable, args: argparse.Namespace, cur_thread=0, num_threads=2):
    num_workers = args.num_workers if args.num_workers is not None else max(1, os.cpu_count() - 1)
    # print(f"totlen:{len(datasets)}")
    for i, dataset in enumerate(datasets):
        ...
        for _, batch in tqdm(dataset.retrieve_latent_cache_batches(num_workers)):
            ...
            for j in range(0, len(batch), bs):
                if args.multi:
                    if j % num_threads != cur_thread:
                        continue
                # print(f"cur_thread:{cur_thread}, num_threads:{num_threads}")
                encode(batch[j : j + bs])
